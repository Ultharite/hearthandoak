/* eslint-disable @next/next/no-img-element */
//import React from 'react'
import { useRef, useState, createContext } from 'react'

const GlobalContext = createContext(true)

import Drawer from '../components/drawer/drawer'
import Button from '../components/button/button'
import ImageHover from '../components/imagehover'
import Navigation from '../components/navigation/navigation'
import Cursor from '../components/cursor/cursor'
import { InView } from 'react-intersection-observer'

const Layout = ({ children }) => {
  const [drawerOpened, setDrawerOpened] = useState(false)
  const toggleDrawer = () => {
    setDrawerOpened(!drawerOpened)
  }

  const setOverButtonTrue = () => {
    setOverButton(true)
  }
  const setOverButtonFalse = () => {
    setOverButton(false)
  }

  const [overButton, setOverButton] = useState(false)
  const [scrolled, setScrolled] = useState(false)

  const ref = useRef(null)

  const topPixelRef = useRef(null)

  return (
    <GlobalContext.Provider
      value={{
        drawerOpened,
        toggleDrawer,
        overButton,
        setOverButton,
        setOverButtonTrue,
        setOverButtonFalse,
      }}
    >
    <InView as="span" className="toppixel" onChange={(inView) => inView == true ? setScrolled(false) : setScrolled(true)} ref={topPixelRef}></InView>
      <main className="site-main" ref={ref}>
        
        
    
        <header className={`siteheader px-2 scrolled-${scrolled}`}>
          <div className="sitelogos">
            <img
              className="sitelogo"
              src="/hando-logo.webp"
              alt="Hearth &amp; Oak"
            />
            <img className="sitelogo-icon" alt="Hearth &amp; Oak" src="/hando-leaf.webp" />
          </div>
          <Navigation />
        </header>
        <Drawer title="Let's Start Your Journey">
          <div className="drawer__content">
            <div className="drawer__contact">
            <h2>Contact Us</h2>
            <p>Call Us:<br /><a href="tel:631-615-0589">631-615-0589</a></p>
            <p>Email Us:<br /><a href="mailto:info@hearthandoak.com">info@hearthandoak.com</a></p>
            <h2>Follow Us</h2>
            <div className="socials">
        <a href="https://www.instagram.com/hearth_and_oak_photo/" target="_blank" rel="noopener noreferrer">
          <svg role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><title>Instagram</title><path d="M12 0C8.74 0 8.333.015 7.053.072 5.775.132 4.905.333 4.14.63c-.789.306-1.459.717-2.126 1.384S.935 3.35.63 4.14C.333 4.905.131 5.775.072 7.053.012 8.333 0 8.74 0 12s.015 3.667.072 4.947c.06 1.277.261 2.148.558 2.913.306.788.717 1.459 1.384 2.126.667.666 1.336 1.079 2.126 1.384.766.296 1.636.499 2.913.558C8.333 23.988 8.74 24 12 24s3.667-.015 4.947-.072c1.277-.06 2.148-.262 2.913-.558.788-.306 1.459-.718 2.126-1.384.666-.667 1.079-1.335 1.384-2.126.296-.765.499-1.636.558-2.913.06-1.28.072-1.687.072-4.947s-.015-3.667-.072-4.947c-.06-1.277-.262-2.149-.558-2.913-.306-.789-.718-1.459-1.384-2.126C21.319 1.347 20.651.935 19.86.63c-.765-.297-1.636-.499-2.913-.558C15.667.012 15.26 0 12 0zm0 2.16c3.203 0 3.585.016 4.85.071 1.17.055 1.805.249 2.227.415.562.217.96.477 1.382.896.419.42.679.819.896 1.381.164.422.36 1.057.413 2.227.057 1.266.07 1.646.07 4.85s-.015 3.585-.074 4.85c-.061 1.17-.256 1.805-.421 2.227-.224.562-.479.96-.899 1.382-.419.419-.824.679-1.38.896-.42.164-1.065.36-2.235.413-1.274.057-1.649.07-4.859.07-3.211 0-3.586-.015-4.859-.074-1.171-.061-1.816-.256-2.236-.421-.569-.224-.96-.479-1.379-.899-.421-.419-.69-.824-.9-1.38-.165-.42-.359-1.065-.42-2.235-.045-1.26-.061-1.649-.061-4.844 0-3.196.016-3.586.061-4.861.061-1.17.255-1.814.42-2.234.21-.57.479-.96.9-1.381.419-.419.81-.689 1.379-.898.42-.166 1.051-.361 2.221-.421 1.275-.045 1.65-.06 4.859-.06l.045.03zm0 3.678c-3.405 0-6.162 2.76-6.162 6.162 0 3.405 2.76 6.162 6.162 6.162 3.405 0 6.162-2.76 6.162-6.162 0-3.405-2.76-6.162-6.162-6.162zM12 16c-2.21 0-4-1.79-4-4s1.79-4 4-4 4 1.79 4 4-1.79 4-4 4zm7.846-10.405c0 .795-.646 1.44-1.44 1.44-.795 0-1.44-.646-1.44-1.44 0-.794.646-1.439 1.44-1.439.793-.001 1.44.645 1.44 1.439z"/></svg>
        </a>
        <a href="https://www.facebook.com/hearthandoakphoto" target="_blank" rel="noopener noreferrer">
          <svg role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><title>Facebook</title><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
        </a>
        </div>
            </div>
            <div className="drawer__form">
              <h2>Book a Session</h2>
              <p className="linewidth">
                Simply fill out the form below and we will promptly get
                back to you so we can start discussing all of the details for your
                big day, and start making memories together. We are looking forward
                to hearing from you so don&apos;t be shy, drop us a line.
              </p>
              <ImageHover>
                <form className="contactform">
                  <div className="contactform__block">
                    <label htmlFor="name">Your Name :</label>
                    <input id="name"></input>
                  </div>
                  <div className="contactform__block">
                    <label htmlFor="name">Your Email :</label>
                    <input id="name"></input>
                  </div>
                  <div className="contactform__block">
                    <label htmlFor="sessiontype">
                      What type of session are you looking for?
                    </label>
                    <div className="radiogroup">
                      <span className="radiooption">
                        <input name="sessiontype" id="wedding" value="wedding" type="radio" />
                        <label htmlFor="wedding">Wedding</label>
                      </span>
                      <span className="radiooption">
                        <input name="sessiontype" id="engagement" value="engagement" type="radio" />
                        <label htmlFor="engagement">Engagement</label>
                      </span>
                      <span className="radiooption">
                        <input name="sessiontype" id="family" value="family" type="radio" />
                        <label htmlFor="family">Family</label>
                      </span>
                      <span className="radiooption">
                        <input name="sessiontype" id="seniors" value="seniors" type="radio" />
                        <label htmlFor="seniors">Seniors</label>
                      </span>
                      <span className="radiooption">
                        <input name="sessiontype" id="event" value="event" type="radio" />
                        <label htmlFor="event">Event</label>
                      </span>
                    </div>
                  </div>
                  <div className="contactform__block">
                  <label htmlFor="reference">How did you hear about us?</label>
                  <select id="reference">
                  <option value="Facebook">Facebook</option>
                  <option value="Google">Google Search</option>
                  <option value="Friend">From a Friend</option>
                  <option value="The Knot">The Knot</option>
                  <option value="Wedding Wire">Wedding Wire</option>
                  <option value="Brides of Long Island">Brides of Long Island</option>
                  <option value="Other">Other</option>
                  </select>
                  </div>
                  <div className="contactform__block">
                    <label htmlFor="name">What would you like to discuss? :</label>
                    <textarea id="name"></textarea>
                  </div>
                  <Button>Send Message</Button>
                </form>
              </ImageHover>
            </div>
          </div>
        </Drawer>
        {children}
      </main>

      <footer className="sitefooter container">
        <div className="sitefooter__content">
        <div className="socials">
        <a href="https://www.instagram.com/hearth_and_oak_photo/" target="_blank" rel="noopener noreferrer">
          <svg role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><title>Instagram</title><path d="M12 0C8.74 0 8.333.015 7.053.072 5.775.132 4.905.333 4.14.63c-.789.306-1.459.717-2.126 1.384S.935 3.35.63 4.14C.333 4.905.131 5.775.072 7.053.012 8.333 0 8.74 0 12s.015 3.667.072 4.947c.06 1.277.261 2.148.558 2.913.306.788.717 1.459 1.384 2.126.667.666 1.336 1.079 2.126 1.384.766.296 1.636.499 2.913.558C8.333 23.988 8.74 24 12 24s3.667-.015 4.947-.072c1.277-.06 2.148-.262 2.913-.558.788-.306 1.459-.718 2.126-1.384.666-.667 1.079-1.335 1.384-2.126.296-.765.499-1.636.558-2.913.06-1.28.072-1.687.072-4.947s-.015-3.667-.072-4.947c-.06-1.277-.262-2.149-.558-2.913-.306-.789-.718-1.459-1.384-2.126C21.319 1.347 20.651.935 19.86.63c-.765-.297-1.636-.499-2.913-.558C15.667.012 15.26 0 12 0zm0 2.16c3.203 0 3.585.016 4.85.071 1.17.055 1.805.249 2.227.415.562.217.96.477 1.382.896.419.42.679.819.896 1.381.164.422.36 1.057.413 2.227.057 1.266.07 1.646.07 4.85s-.015 3.585-.074 4.85c-.061 1.17-.256 1.805-.421 2.227-.224.562-.479.96-.899 1.382-.419.419-.824.679-1.38.896-.42.164-1.065.36-2.235.413-1.274.057-1.649.07-4.859.07-3.211 0-3.586-.015-4.859-.074-1.171-.061-1.816-.256-2.236-.421-.569-.224-.96-.479-1.379-.899-.421-.419-.69-.824-.9-1.38-.165-.42-.359-1.065-.42-2.235-.045-1.26-.061-1.649-.061-4.844 0-3.196.016-3.586.061-4.861.061-1.17.255-1.814.42-2.234.21-.57.479-.96.9-1.381.419-.419.81-.689 1.379-.898.42-.166 1.051-.361 2.221-.421 1.275-.045 1.65-.06 4.859-.06l.045.03zm0 3.678c-3.405 0-6.162 2.76-6.162 6.162 0 3.405 2.76 6.162 6.162 6.162 3.405 0 6.162-2.76 6.162-6.162 0-3.405-2.76-6.162-6.162-6.162zM12 16c-2.21 0-4-1.79-4-4s1.79-4 4-4 4 1.79 4 4-1.79 4-4 4zm7.846-10.405c0 .795-.646 1.44-1.44 1.44-.795 0-1.44-.646-1.44-1.44 0-.794.646-1.439 1.44-1.439.793-.001 1.44.645 1.44 1.439z"/></svg>
        </a>
        <a href="https://www.facebook.com/hearthandoakphoto" target="_blank" rel="noopener noreferrer">
          <svg role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><title>Facebook</title><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
        </a>
        </div>
        <Button onClick={toggleDrawer}>Book a Session</Button>
        </div>
        <span className="copyright">
          All content copyright &copy; 2022 Hearth &amp; Oak Photography Company
        </span>
      </footer>
    </GlobalContext.Provider>
  )
}

export { GlobalContext }

export default Layout
